...
module.exports.handleReply = async function({ api, event, handleReply }) {
	...
	api.sendMessage(`Đang xử lý audio...`, event.threadID, (err, info) =>
	...
	if (urlValid) {
		...
		try {
			...
			for (let value of results) {
				...
				msg += (`${num+=1}. <${channel}> ${value.title}\n\n`);
			}
			return api.sendMessage(msg, event.threadID,(error, info) => global.client.handleReply.push({ name: this.config.name, messageID: info.messageID, author: event.senderID, link }), event.messageID);
		}
		catch (error) {
			api.sendMessage("Không thể xử lý request do dã phát sinh lỗi: " + error.message, event.threadID, event.messageID);
		}
