...
module.exports.run = async function({ api, event, args, Currencies, getText, permssion }) {
  try {
    ...
    if (this.config.credits != 'jutsu') {
      console.log('\x1b[33m[ WARN ]\x1b[37m » Đổi credits ???');
    ...
    if (!existsSync(__dirname + '/cache/ga.jpg')) {
      request('https://i.ibb.co/CVPB4k2/ga.jpg').pipe(createWriteStream(__dirname + '/cache/ga.jpg'));
    }
    if (!existsSync(__dirname + '/cache/tom.jpg')) {
      request('https://i.ibb.co/c6PZbXv/tom.jpg').pipe(createWriteStream(__dirname + '/cache/tom.jpg'));
    }
    if (!existsSync(__dirname + '/cache/bau.jpg')) {
      request('https://i.ibb.co/6bwZNWz/bau.jpg').pipe(createWriteStream(__dirname + '/cache/bau.jpg'));
    }
    if (!existsSync(__dirname + '/cache/cua.jpg')) {
      request('https://i.ibb.co/k39J791/cua.jpg').pipe(createWriteStream(__dirname + '/cache/cua.jpg'));
    }
    if (!existsSync(__dirname + '/cache/ca.jpg')) {
      request('https://i.ibb.co/yhwsVLh/ca.jpg').pipe(createWriteStream(__dirname + '/cache/ca.jpg'));
    }
    if (!existsSync(__dirname + '/cache/nai.jpg')) {
      request('https://i.ibb.co/LZdkbjb/nai.jpg').pipe(createWriteStream(__dirname + '/cache/nai.jpg'));
    }
    if (!existsSync(__dirname + '/cache/baucua.gif')) {
      request('https://i.ibb.co/C88t0kj/baucua.gif').pipe(createWriteStream(__dirname + '/cache/baucua.gif'));
    }
    // baucua 1
    if (baucua1 == 'gà') {
      var bau1 = 'ga';
